...
object Rand04Functions extends App {
  ...
  val nextLong: RNG => (RNG, Long) = rng => rng.nextLong

  val nextInt: RNG => (RNG, Int) = rng => {
  ...
  val nextDouble: RNG => (RNG, Double) = rng => {
    val (newRng, i) = nonNegativeInt(rng)
    val d = i / (Int.MaxValue.toDouble + 1)
    (newRng, d)
  }

  val nextBoolean: RNG => (RNG, Boolean) = rng => {
    val (newRng, i) = nextInt(rng)
    (newRng, i % 2 == 0)
  }

  val nextIntPair: RNG => (RNG, (Int, Int)) = rng => {
  ...
  def rollDieNTimes(times: Int): RNG => (RNG, List[Int]) = rng => {
    if (times <= 0)
      (rng, List.empty[Int])
    else {
